...
public class OllamaQuizGenerator implements QuizGenerator {
    ...
    public List<GeneratedQuestion> generate(PokemonDto pokemon, PokemonSpeciesDto species) {
        ...
        if (species != null && species.evolutionChainUrl() != null) {
            try {
                chain = pokeApi.getEvolutionChain(species.evolutionChainUrl());
            } catch (Exception e) {
                // Non-fatal: continue without chain data
            }
        }
        ...
        try {
            ...
            throw new QuizGenerationException("Ollama call failed: " + e.getMessage(), e);
        }

        // Strip accidental code fences if the model wraps JSON in ```json ... ```
        ...
        try {
            ...
            throw new QuizGenerationException(
                "Failed to parse Ollama JSON response: " + e.getMessage() + " (raw: "
